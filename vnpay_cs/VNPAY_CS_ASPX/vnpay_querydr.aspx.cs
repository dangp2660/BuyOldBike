...
namespace VNPAY_CS_ASPX
{
    public partial class vnpay_querydr : System.Web.UI.Page
    {
        ...
        public void btnQuery_Click(object sender, EventArgs e)
        {
            ...
            var vnp_Api = (ConfigurationManager.AppSettings["vnp_Api"] ?? string.Empty).Trim().Trim('`').Trim();
            var vnp_HashSecret = (ConfigurationManager.AppSettings["vnp_HashSecret"] ?? string.Empty).Trim().Trim('`').Trim(); //Secret KEy
            var vnp_TmnCode = (ConfigurationManager.AppSettings["vnp_TmnCode"] ?? string.Empty).Trim().Trim('`').Trim(); // Terminal Id
